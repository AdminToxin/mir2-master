    ...
    public sealed class GameScene : MirScene
    {
                ...
                for (int i = MapControl.Objects.Count - 1; i >= 0; i--)
                {
                    ...
                    switch (p.Type)
                    {
                        case DamageType.Hit: //add damage level colours
                            obj.Damages.Add(new Damage(p.Damage.ToString("#,##0"), 1000, obj.Race == ObjectType.Player ? Color.Yellow : Color.White, 50));
                            break;
                        case DamageType.Miss:
                            obj.Damages.Add(new Damage("Miss", 1200, obj.Race == ObjectType.Player ? Color.Pink : Color.White, 50));
                            break;
                ...
                switch (p.Effect)
                {
                   ...
                   case SpellEffect.MPEater:
                    ...
                    case SpellEffect.MagicShieldUp:
                        ...
                        player.Effects.Add(player.ShieldEffect = new Effect(Libraries.Magic, 4058, 3, 600, ob) { Repeat = true });
                        break;
                    ...
                    case SpellEffect.Frozen:
                        ob.Effects.Add(new Effect(Libraries.Magic4, 2520, 70, 6600, ob) { Repeat = true});
                        //{
                        //    Repeat = p.Time > 0,
                        //    RepeatUntil = p.Time > 0 ? CMain.Time + p.Time : 0
                        //});
        ...
        public MirControl AttackInfoLabel(UserItem item, bool Inspect = false)
        {
            ...
            if (minValue > 0 || maxValue > 0 || addValue > 0)
            {
                ...
                {
                    AutoSize = true,
                    ForeColour = addValue > 0 ? Color.Green : Color.White,
                    Location = new Point(4, ItemLabel.DisplayRectangle.Bottom),
                    OutLine = true,
                    Parent = ItemLabel,
        ...
        public override string ToString()
        {
            ...
            switch (Type)
            {
                ...
                case BuffType.Knapsack:
                    ...
                    break;
                case BuffType.Rebirth:
                    text = string.Format("Rebirth\n");

                    if (Values.Length > 0)
                        text += string.Format("HP increased by {0}\n", Values[0]);
                    if (Values.Length > 1)
                        text += string.Format("MP increased by {0}\n", Values[1]);
                    break;
            }
