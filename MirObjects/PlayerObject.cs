...
namespace Server.MirObjects
{
    public sealed class PlayerObject : MapObject
    {
        ...
        public void SetLevelEffects()
        {
            ...
            //if (Info.Flags[993]) LevelEffects |= LevelEffects.RedAura;
            if (Info.Flags[993]) LevelEffects |= LevelEffects.Mir3White;
            if (Info.Flags[994]) LevelEffects |= LevelEffects.YellowAura;
            if (Info.Flags[995]) LevelEffects |= LevelEffects.BlueAura;
            if (Info.Flags[996]) LevelEffects |= LevelEffects.PurpleAura;
            if (Info.Flags[997]) LevelEffects |= LevelEffects.Shields;
        }
        ...
        private void RefreshEquipmentStats()
        {
            ...
            short OldLooks_WeaponEffect = Looks_WeaponEffect; //weaponeffects
            ...
            Looks_Armour = 0;
            Looks_Weapon = -1;
            Looks_WeaponEffect = 0; //weaponeffects
            Looks_Wings = 0;
            Light = 0;
            CurrentWearWeight = 0;
            CurrentHandWeight = 0;
            ...
            for (int i = 0; i < Info.Equipment.Length; i++)
            {
                ...
                if (RealItem.Type == ItemType.Weapon)
                {
                    ...
                    Looks_WeaponEffect = RealItem.Effect; //weaponeffects
                }
            ...
            if ((OldLooks_Armour != Looks_Armour) || (OldLooks_Weapon != Looks_Weapon) || (OldLooks_WeaponEffect != Looks_WeaponEffect) || (OldLooks_Wings != Looks_Wings) || (OldLight != Light)) //weaponeffects
            {
        ...
        public void Magic(Spell spell, MirDirection dir, uint targetID, Point location)
        {
            ...
            switch (spell)
            {
                ...
                case Spell.UnlimitedBladeWorks:
                    UnlimitedBladeWorks(target, magic);
                    break;
        ...
        public void UnlimitedBladeWorks(MapObject target, UserMagic magic)
        {
            int damage = magic.GetDamage(GetAttackPower(MinDC, MaxDC));
                ...
                case Spell.FrostCrunch:
                    ...
                    if (target.Attacked(this, value, DefenceType.MAC, false) > 0)
                    {
                        ...
                        if (Level + (target.Race == ObjectType.Player ? 2 : 10) >= target.Level && Envir.Random.Next(target.Race == ObjectType.Player ? 100 : 40) <= magic.Level)
                        {
                            ...
                            target.OperateTime = 0;
                            //target.AddBuff(new Buff { Type = BuffType.Frozen, Caster = this, ExpireTime = 5, Visible = true });
                        }
        ...
        private Packet GetUpdateInfo()
        {
            ...
            return new S.PlayerUpdate
            {
                ObjectID = ObjectID,
                Weapon = Looks_Weapon,
                WeaponEffect = Looks_WeaponEffect, //weaponeffects
                Armour = Looks_Armour,
                Light = Light,
                ...
                case ItemType.Potion:
                    switch (item.Info.Shape)
                    {
                        ...
                        case 3: //Buff
                            ...
                            if ((item.Info.HP + item.HP) > 0)
                                AddBuff(new Buff { Type = BuffType.Rebirth, Caster = this, ExpireTime = Envir.Time + time * Settings.Minute, Values = new int[] { item.Info.HP + item.HP } });

                            if ((item.Info.MP + item.MP) > 0)
                                AddBuff(new Buff { Type = BuffType.Rebirth, Caster = this, ExpireTime = Envir.Time + time * Settings.Minute, Values = new int[] { item.Info.MP + item.MP } });
                            break;
                        case 4: //Exp
